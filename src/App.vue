...
import TMDBPosterWall from '@/components/TMDBPosterWall.vue';
...
// 当前页面状态
const currentPage = ref<'search' | 'poster' | 'status' | 'docs' | 'accounts' | 'qqpd' | 'gying' | 'panlian' | 'weibo'>('search');
...
// 页面切换
const switchToPoster = () => {
  currentPage.value = 'poster';
};
...
<template>
  <div class="app-shell min-h-screen bg-background text-foreground transition-colors duration-300 flex flex-col">
      ...
      <div class="container mx-auto px-4 h-16 flex items-center justify-between">
        ...
        <nav class="flex items-center gap-2">
          ...
          <button 
            ...
            class="nav-button"
            :class="{ 'active': currentPage === 'poster' }"
            title="海报墙"
          >
            <span class="nav-icon">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    >
      ...
      <!-- TMDB海报墙页面 -->
      <div v-else-if="currentPage === 'poster'" class="poster-page">
        <TMDBPosterWall :backend-health="backendHealth" />
      </div>
...
/* 页面切换动画 */
.search-page, .poster-page, .status-page {
